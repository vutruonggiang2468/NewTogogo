...
export default function CashFlow({ data, loading, error }: Props) {
  ...
  const Cell = (
    ...
    return (
      <td
        className={`p-4 text-right ${amountClass(v)} ${
          strong ? "font-bold" : ""
        }`}
      >
        {fmtBnVND(v)}
  ...
  const MetricRow: React.FC<Metric> = ({
    ...
    return (
      <tr className="hover:bg-slate-700/20 transition-colors border-b border-slate-700/40">
        <td
          className={`p-4 pl-8 ${
            strong ? "text-slate-200 font-medium" : "text-slate-300"
          }`}
        >
          {label}
        </td>
        ...
        <CardContent className="p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex gap-3 flex-wrap">
              <Badge className="bg-gradient-to-r from-emerald-500/20 to-emerald-600/10 text-emerald-400 px-4 py-2 border border-emerald-400/30">
                <CheckCircle className="w-3 h-3 mr-2" />Q{selectedQuarter}{" "}
                {selectedYear}
              </Badge>
          ...
          {(() => {
            ...
            const generateBalanceSheetData = (
              ...
              const yearVariation =
                { 2023: 0.7, 2024: 0.85, 2025: 1.0, 2026: 1.2 }[year] || 1.0;
              ...
              const data = {
                ...
                symbol: {
                  ...
                  company: {
                    ...
                    outstanding_share: 80000000,
                    charter_capital: 800000000000,
                  },
                },
                current_assets: Math.round(687434000000 * adjustedFactor),
                cash_and_cash_equivalents: Math.round(
                  139000000 * adjustedFactor * 1.2
                ),
                short_term_investments: Math.round(1120000 * adjustedFactor),
                accounts_receivable: Math.round(
                  686000000000 * adjustedFactor * 0.95
                ),
                net_inventories: Math.round(1130000000 * adjustedFactor),
                prepayments_to_suppliers: Math.round(
                  23520000000 * adjustedFactor
                ),
                other_current_assets: 0,
                long_term_assets: Math.round(
                  13550000000 * adjustedFactor * 1.1
                ),
                fixed_assets: Math.round(9400000000 * adjustedFactor * 1.1),
                long_term_investments: Math.round(
                  adjustedFactor > 1 ? 2000000000 * (adjustedFactor - 1) : 0
                ),
                ...
                current_liabilities: Math.round(
                  78880000000 * adjustedFactor * 0.9
                ),
                short_term_borrowings: Math.round(
                  61240000000 * adjustedFactor * 0.85
                ),
                advances_from_customers: Math.round(
                  17640000000 * adjustedFactor
                ),
                long_term_liabilities: Math.round(
                  584000000 * adjustedFactor * 0.8
                ),
                long_term_borrowings: Math.round(
                  584000000 * adjustedFactor * 0.8
                ),
                owners_equity: 0,
                ...
                undistributed_earnings: Math.round(
                  -221100000000 + (adjustedFactor - 1) * 300000000000
                ),
                investment_and_development_funds: Math.round(
                  59520000 * adjustedFactor
                ),
                total_resources: 0,
              };
            ...
            const generateCashFlowData = (year: number, quarter: number) => {
              ...
              const totalFactor = yearFactor * quarterFactor;
              const yearVariation =
                { 2023: 0.75, 2024: 0.88, 2025: 1.0, 2026: 1.15 }[year] || 1.0;
              const adjustedFactor = totalFactor * yearVariation;

              return {
                net_income: Math.round(45000000000 * adjustedFactor),
                depreciation_amortization: Math.round(
                  8500000000 * adjustedFactor
                ),
                working_capital_changes: Math.round(
                  -12000000000 * adjustedFactor * 0.8
                ),
                accounts_receivable_change: Math.round(
                  -35000000000 * adjustedFactor * 0.9
                ),
                inventory_change: Math.round(-2500000000 * adjustedFactor),
                accounts_payable_change: Math.round(
                  18000000000 * adjustedFactor
                ),
                other_operating_activities: Math.round(
                  3200000000 * adjustedFactor
                ),
                capital_expenditures: Math.round(
                  -8500000000 * adjustedFactor * 1.1
                ),
                business_acquisitions: Math.round(
                  adjustedFactor > 1.1 ? -15000000000 * (adjustedFactor - 1) : 0
                ),
                ...
                other_investing_activities: Math.round(
                  -1200000000 * adjustedFactor
                ),
                debt_issuance: Math.round(25000000000 * adjustedFactor * 0.7),
                debt_repayment: Math.round(-35000000000 * adjustedFactor),
                equity_issuance: Math.round(
                  adjustedFactor > 1.05 ? 10000000000 * (adjustedFactor - 1) : 0
                ),
                dividend_payments: Math.round(
                  -8500000000 * adjustedFactor * 1.05
                ),
                share_repurchases: Math.round(-2800000000 * adjustedFactor),
                other_financing_activities: Math.round(
                  -1500000000 * adjustedFactor
                ),
                beginning_cash: Math.round(125000000 * adjustedFactor * 0.9),
                foreign_exchange_effect: Math.round(
                  850000000 * adjustedFactor * 0.3
                ),
              };
            };
            ...
            const generateFinancialData = (
              ...
              const revenue =
                248000000000 *
                (1 +
                  (selectedYear - 2025) * 0.12 +
                  (selectedQuarter - 2) * 0.03);
              ...
              const interestExpense =
                4200000000 * (1 + (selectedYear - 2025) * 0.08);
              const totalBorrowing =
                balanceSheet.short_term_borrowings +
                balanceSheet.long_term_borrowings;
            ...
            const operatingCashFlow =
              ...
              cashFlowData.depreciation_amortization +
              cashFlowData.working_capital_changes +
              cashFlowData.other_operating_activities;

            // Calculate all financial ratios
            const ratios = {
              // Capital Structure Ratios
              totalDebtToEquity:
                (balanceSheetData.short_term_borrowings +
                  balanceSheetData.long_term_borrowings) /
                balanceSheetData.owners_equity,
              debtToEquity:
                financialData.totalBorrowing / balanceSheetData.owners_equity,
              fixedAssetToEquity:
                balanceSheetData.fixed_assets / balanceSheetData.owners_equity,
              equityToCharterCapital:
                balanceSheetData.owners_equity /
                balanceSheetData.paid_in_capital,

              // Efficiency Ratios
              assetTurnover:
                financialData.revenue / balanceSheetData.total_assets,
              fixedAssetTurnover:
                financialData.revenue / balanceSheetData.fixed_assets,
              daysSalesOutstanding:
                (balanceSheetData.accounts_receivable / financialData.revenue) *
                365,
              daysInventoryOutstanding:
                (balanceSheetData.net_inventories /
                  (financialData.revenue * 0.72)) *
                365,
              daysPayableOutstanding:
                ((balanceSheetData.current_liabilities * 0.6) /
                  (financialData.revenue * 0.72)) *
                365,
              inventoryTurnover:
                (financialData.revenue * 0.72) /
              ...
              grossProfitMargin:
                (financialData.grossProfit / financialData.revenue) * 100,
              netProfitMargin:
                (financialData.netIncome / financialData.revenue) * 100,
              roe:
                (financialData.netIncome / balanceSheetData.owners_equity) *
                100,
              roic:
                ((financialData.ebit * 0.75) /
                  (balanceSheetData.owners_equity +
                    financialData.totalBorrowing)) *
                100,
              roa:
                (financialData.netIncome / balanceSheetData.total_assets) * 100,
              dividendYield:
                (financialData.dividendPerShare / financialData.marketPrice) *
                100,
              ...
              cashRatio:
                balanceSheetData.cash_and_cash_equivalents /
                balanceSheetData.current_liabilities,
              quickRatio:
                (balanceSheetData.current_assets -
                  balanceSheetData.net_inventories) /
                balanceSheetData.current_liabilities,
              interestCoverage:
                financialData.ebit / financialData.interestExpense,
              financialLeverage:
                balanceSheetData.total_assets / balanceSheetData.owners_equity,

              // Valuation Ratios
              pe: financialData.marketPrice / financialData.eps,
              pb:
                financialData.marketPrice /
                (balanceSheetData.owners_equity /
                  balanceSheetData.common_shares),
              ...
              <div className="space-y-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                      ...
                      <div className="text-sm text-slate-400">
                        {(() => {
                          ...
                          const endDate =
                            quarterEndDates[
                              selectedQuarter as keyof typeof quarterEndDates
                            ];
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-600/50">
                      ...
                      <div className="text-xl font-bold text-cyan-400 mb-1">
                        {(financialData.outstandingShares / 1000000).toFixed(1)}
                        M
                      </div>
                      <div className="text-xs text-slate-500">
                ...
                <div className="p-6 bg-gradient-to-r from-slate-900/80 to-slate-800/60 rounded-xl border border-slate-500/50">
                  <h5 className="font-semibold text-white mb-4 flex items-center gap-3">
                    ...
                    Đánh giá tổng quan chỉ số tài chính (Financial Ratios
                    Summary)
                  </h5>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <h6 className="font-medium text-emerald-400">
                        Điểm mạnh (Strengths)
                      </h6>
                      <ul className="space-y-2 text-sm text-slate-300">
                        <li className="flex items-start gap-2">
                          ...
                          <span>
                            Tỷ suất sinh lời ROE cao {ratios.roe.toFixed(1)}%
                            cho thấy hiệu quả sử dụng vốn tốt
                          </span>
                        </li>
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full mt-2 flex-shrink-0"></div>
                          <span>
                            Khả năng thanh toán lãi vay mạnh với hệ số{" "}
                            {ratios.interestCoverage.toFixed(1)}x
                          </span>
                        </li>
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full mt-2 flex-shrink-0"></div>
                          <span>
                            Tỷ lệ thanh khoản hiện hành ổn định tại{" "}
                            {ratios.currentRatio.toFixed(2)}x
                          </span>
                        </li>
                      </ul>
                    ...
                    <div className="space-y-3">
                      <h6 className="font-medium text-orange-400">
                        Cần chú ý (Areas to Watch)
                      </h6>
                      <ul className="space-y-2 text-sm text-slate-300">
                        <li className="flex items-start gap-2">
                        ...
                        <li className="flex items-start gap-2">
                          ...
                          <span>
                            Đòn bẩy tài chính{" "}
                            {ratios.financialLeverage.toFixed(2)}x cần kiểm soát
                            rủi ro
                          </span>
                        </li>
                        <li className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-orange-400 rounded-full mt-2 flex-shrink-0"></div>
                          <span>
                            P/E ratio {ratios.pe.toFixed(1)}x so với ngành cần
                            theo dõi
                          </span>
                        </li>
                      </ul>
